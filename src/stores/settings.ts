...
> = {
  ...
  anthropic: {
    ...
    models: [
      { id: "claude-3-5-sonnet-latest", name: "Claude 3.5 Sonnet", enabled: true },
      { id: "claude-3-5-haiku-latest", name: "Claude 3.5 Haiku", enabled: true },
      { id: "claude-3-opus-latest", name: "Claude 3 Opus", enabled: false },
    ],
  },
  google: {
  ...
  deepseek: {
    ...
    models: [
      ...
      { id: "deepseek-reasoner", name: "DeepSeek Reasoner", enabled: false },
    ],
  },
  qwen: {
...
function loadSettings(): ModelSettings {
  try {
    ...
    const settings: ModelSettings = {
      ...
      providers: { ...DEFAULT_SETTINGS.providers, ...(parsed.providers as any) },
      customProviders: parsed.customProviders ?? [],
    };

    // 确保每个提供商都有配置
    (Object.keys(PROVIDER_LABELS) as ModelProvider[]).forEach(provider => {
      if (!settings.providers[provider]) {
        settings.providers[provider] = createDefaultProviderConfig(provider);
      }
    });
...
export const useSettingsStore = defineStore("settings", () => {
  ...
  function updateProviderConfig(provider: ModelProvider, updates: Partial<ProviderConfig>) {
    const config = model.value.providers[provider];
    if (config) {
      Object.assign(config, updates);
      // 如果更新的是当前使用的提供商,同步更新主配置
      if (provider === model.value.provider) {
        if (updates.apiKey !== undefined) model.value.apiKey = updates.apiKey;
        if (updates.baseUrl !== undefined) model.value.baseUrl = updates.baseUrl;
        if (updates.activeModel !== undefined) model.value.model = updates.activeModel;
      }
    }
  }
  ...
  function removeCustomProvider(id: string) {
    model.value.customProviders = model.value.customProviders.filter(p => p.id !== id);
  }
