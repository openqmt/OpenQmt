...
export const useAuthStore = defineStore("auth", () => {
  ...
  async function login(email: string, password: string): Promise<AuthResult> {
    ...
    try {
      ...
      if (!found || found.password !== password) {
        return { success: false, message: "邮箱或密码错误", user: null, token: null };
      }
  ...
  async function register(email: string, password: string): Promise<AuthResult> {
    ...
    try {
      if (!email.includes("@") || !email.includes(".")) {
        return { success: false, message: "邮箱格式不正确", user: null, token: null };
      }
      if (password.length < 6) {
        return { success: false, message: "密码至少6位", user: null, token: null };
      }
      const users = loadMockUsers();
      if (users.find((u) => u.email === email)) {
        return { success: false, message: "该邮箱已注册", user: null, token: null };
      }
    ...
    try {
      ...
      if (!found) {
        found = {
          ...
          password: "",
          nickname: "GitHub用户",
          avatar_url: "https://api.dicebear.com/7.x/identicon/svg?seed=" + githubId,
          github_id: githubId,
          credits: 100,
          created_at: new Date().toISOString(),
        };
      ...
      token.value = tk;
      saveSession({ userId: found.id, token: tk });
      return { success: true, message: "GitHub 登录成功", user: info, token: tk };
    } finally {
      loading.value = false;
    }
  }
  ...
  async function updateNickname(nickname: string): Promise<AuthResult> {
    if (!user.value) return { success: false, message: "未登录", user: null, token: null };
    ...
    if (found) {
      ...
      const info = userToInfo(found);
      user.value = info;
      return { success: true, message: "昵称修改成功", user: info, token: null };
    }
    return { success: false, message: "用户不存在", user: null, token: null };
  }

  /** 修改密码 */
  async function updatePassword(oldPassword: string, newPassword: string): Promise<AuthResult> {
    if (!user.value) return { success: false, message: "未登录", user: null, token: null };
    if (newPassword.length < 6) return { success: false, message: "新密码至少6位", user: null, token: null };
    await new Promise((r) => setTimeout(r, 300));
    const users = loadMockUsers();
    const found = users.find((u) => u.id === user.value!.id);
    if (found) {
      ...
      if (found.password && found.password !== oldPassword) {
        return { success: false, message: "旧密码错误", user: null, token: null };
      }
      found.password = newPassword;
      saveMockUsers(users);
      return { success: true, message: "密码修改成功", user: null, token: null };
    }
    return { success: false, message: "用户不存在", user: null, token: null };
  }
  ...
  async function rechargeCredits(amount: number): Promise<AuthResult> {
    if (!user.value) return { success: false, message: "未登录", user: null, token: null };
    if (amount <= 0) return { success: false, message: "充值金额必须大于0", user: null, token: null };
    await new Promise((r) => setTimeout(r, 500));
    const users = loadMockUsers();
    const found = users.find((u) => u.id === user.value!.id);
    if (found) {
      ...
      const info = userToInfo(found);
      user.value = info;
      return { success: true, message: `充值成功，当前积分: ${found.credits}`, user: info, token: null };
    }
    return { success: false, message: "用户不存在", user: null, token: null };
  }
